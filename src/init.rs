...
pub fn initialize_project(project: &Path, choice: EngineChoice) -> Result<Vec<String>> {
    ...
    let contrato = project.join(".engine-sensor");
    fs::create_dir_all(&contrato).with_context(|| format!("não consegui criar {}", contrato.display()))?;
    let engine_dir = contrato.join(engine.to_string());
    fs::create_dir_all(&engine_dir)
        .with_context(|| format!("não consegui criar {}", engine_dir.display()))?;

    let mut created = Vec::new();
    write_new(&contrato.join("CONTRATO.md"), CONTRACT, project, &mut created)?;
    write_new(
        &contrato.join("AGENTS.fragment.md"),
        AGENTS_FRAGMENT,
        project,
        &mut created,
    )?;
    write_new(
        &contrato.join("CLAUDE.fragment.md"),
        CLAUDE_FRAGMENT,
        project,
        &mut created,
    )?;
    write_new(&contrato.join("EVIDENCIA.md"), EVIDENCE, project, &mut created)?;
    write_new(&contrato.join("PADROES.md"), PATTERNS, project, &mut created)?;
    write_new(&contrato.join("USOS.md"), USES, project, &mut created)?;
    match engine {
    ...
    write_new(&project.join("engine-sensor.toml"), &config, project, &mut created)?;
    Ok(created)
}

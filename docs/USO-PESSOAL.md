...
> Este é o registro **do próprio engine-sensor**. O modelo distribuído por `engine-sensor init` está em
> [`kit/USOS.md`](../kit/USOS.md); a instância criada em cada projeto integrado fica em
> `.engine-sensor/USOS.md` daquele projeto. Três arquivos, três papéis distintos.
...
**O engine-sensor muda durante o marco, e isso é o método — não um descuido.** Pela
[ADR 0012](decisoes/0012-o-sensor-e-o-corpus-coevoluem.md), a obrigação deixou de ser "não
altere o instrumento" e passou a ser "não altere o instrumento sem deixar evidência". Por
isso a coluna **engine-sensor** existe: ela diz qual instrumento respondeu a cada caso. Quando um
...
> [ADR 0012 §5](decisoes/0012-o-sensor-e-o-corpus-coevoluem.md) mandou medir.
...
mediu foi o engine-sensor acompanhando um jogo, não o engine-sensor acompanhando um corpus.
...
| # | Data | **engine-sensor** | Projeto | Mudança | Tempo | Conflito | Aviso útil/falso | Inspeção humana necessária | Capacidade ausente |
|---:|---|---|---|---|---:|---|---|---|---|
...
`set_speed_scale`. O engine-sensor varreu 1142 arquivos e deu saída 0 sobre um mecanismo cuja regra
...
[ADR 0012 §5](decisoes/0012-o-sensor-e-o-corpus-coevoluem.md) saindo neutra.
...
### Nota 2 — o uso 13 alterou o engine-sensor de novo: a profundidade de desenho
...
**O que faltava.** O engine-sensor modelava quem **anima** uma propriedade. Não modelava quem
...
`Node.move_child`. É a forma que o engine-sensor já usa para duas fontes de verdade sobre uma
...
3. **O pai continua fora.** `z_index` é relativo ao pai, e o pai é informação de cena
   (`.tscn`), que o engine-sensor não lê. Por isso cada declaração é sobre **um** nó e nunca sobre
...
O engine-sensor pode permanecer privado ao final. Nova etapa pública exige nova decisão;
não é continuação automática deste registro.

## O uso 2 e sua procedência

O uso 2 não veio do trabalho normal: veio de uma regra que o próprio engine-sensor acabara de
...
**O engine-sensor não achou nada, e não é silêncio de rotina.** Saída 0, 302 claims em 1142
...
**As três capacidades ausentes são de animação, que é a família que o engine-sensor já modela** — e
por isso valem mais que hipótese sobre eixo novo. Foram nomeadas pelo diário do próprio
porte, nas linhas 11, 12 e 13 de `.engine-sensor/USOS.md`:

1. **`pause()`/`play()` num Tween não entra no inventário.** O engine-sensor vê a propriedade e o
...
2. **`set_speed_scale` também não.** O engine-sensor modela quem *anima* uma propriedade, não o
...
[ADR 0012 §3](decisoes/0012-o-sensor-e-o-corpus-coevoluem.md) exige o confronto antes da
incorporação.
...
estavam registradas só no diário do porte, em `.engine-sensor/USOS.md`, e nunca subiram para cá.
...
**O MineBoom não foi integrado, e isso é deliberado.** Ele não tem `.engine-sensor/` nem
`engine-sensor.toml`. Integrar só serve para projeto que vai gerar mudança real, e ele está
...
O engine-sensor não passou a ver mais animação; passou a haver mais animação.
...
em `.engine-sensor/USOS.md`, é mantido pela sessão que trabalha lá.

## Baseline do Gods: o único aviso, classificado

`ESN-OWN-001` em `animation:godot:src/entities/card.gd:self:position`, apontando
...
a remediação que o próprio `ESN-OWN-001` imprime é *"centralize o proprietário"*, e é
...
## Observação: as onze pegadinhas do porte, e quantas o engine-sensor pegaria
...
**E o engine-sensor não pegaria nenhuma das onze.** Nenhuma é conflito de posse: `mouse_filter` é
...
doze porque o engine-sensor o achou antes. Uma lista de pegadinhas só registra o que passou.
...
> capazes de derrubá-la —, e produziram **um aviso útil** (uso 6, o `ESN-PARSE-001` da
...
**Seis defeitos na classe "só olho humano acha". O engine-sensor cobre dois.** Um terço, com
amostra de seis.

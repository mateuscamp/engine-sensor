# ADR 0007 - `engine-sensor observe` nasce como binário separado
...
Se `observe` entrar no mesmo binário, `engine-sensor check` herda a dependência de uma
...
1. Subcomando `engine-sensor observe` no mesmo binário.
2. Feature de Cargo desligada por padrão, um binário só.
3. Segundo binário `engine-sensor-observe` no mesmo workspace, compartilhando a biblioteca.
...
- `engine-sensor-observe` é um segundo `[[bin]]` do mesmo crate, ou um segundo crate do mesmo
  workspace, compartilhando `model` e `report` pela biblioteca.
- O binário `engine-sensor` continua sendo um quantum independente: sem Godot, sem rede, sem
  runtime externo. O teste offline e a lista de dependências autorizadas aplicam-se a
  ele.
- `engine-sensor-observe` pode exigir Godot instalado. Essa exigência é dele, não do portão.
- O contrato estrito da [ADR 0006](0006-contrato-estrito-de-relatorio-e-codigos-de-saida.md)
  cobre o relatório de `engine-sensor check`. A evidência produzida por `observe` tem forma
...
`[[bin]]` fora da lista `engine-sensor` e `engine-sensor-observe`.

*(Acrescentado em 30/08/2026, porque a distinção ficou invisível por meses: **`engine-sensor-observe`
nunca existiu como artefato.** O `Cargo.toml` declara um `[[bin]]` só, o `engine-sensor`. Esta ADR
...
nenhum binário novo nascesse. Ler a lista `engine-sensor` e `engine-sensor-observe` como inventário é o
mesmo erro que este projeto existe para nomear — decisão registrada não é artefato
medido.)*

Quando `engine-sensor-observe` existir, a lista de dependências autorizadas passa a ser por
binário e o teste offline continua sendo executado contra `engine-sensor`. Enquanto ele não
